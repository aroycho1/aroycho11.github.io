...
const EducationSection = () => {
  ...
  return <section id="education" className="section-container">
      <div className="max-w-4xl mx-auto" ref={ref}>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {education.map((edu, index) => <motion.div key={edu.university} initial={{
          opacity: 0,
          y: 50
        }} animate={isInView ? {
          opacity: 1,
          ...
          ease: "easeOut"
              ...
              <div className="absolute top-4 right-4">
                <img src={edu.flag} alt={`${edu.country} flag`} className="w-8 h-5 rounded object-cover shadow-sm" />
              </div>

              <div className="flex items-start gap-4">
                {/* University Logo */}
                <div className="flex-shrink-0 w-16 h-16 rounded-xl bg-muted flex items-center justify-center overflow-hidden">
                  <img src={edu.logo} alt={`${edu.university} logo`} className="w-12 h-12 object-contain" />
                </div>
